...
def user_can_see(context, *args, **kwargs):
    ...
    return app.public or ArchiverApp.objects.filter(members=context['user'].id, app_label=app_label) or ArchiverApp.objects.filter(creators=context['user'].id, app_label=app_label)
